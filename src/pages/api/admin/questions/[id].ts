...
import { updateQuestion, deleteQuestion } from '../../../../lib/store';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    if (req.method === 'PUT') {
      ...
      if (!updated) {
        return res.status(404).json({ success: false, data: null, message: 'Không tìm thấy câu hỏi' });
      }
      return res.status(200).json({ success: true, data: updated, message: 'Đã cập nhật câu hỏi' });
    }
    ...
    if (req.method === 'DELETE') {
      ...
      return res.status(200).json({ success: true, data: null, message: 'Đã xóa câu hỏi' });
    }
