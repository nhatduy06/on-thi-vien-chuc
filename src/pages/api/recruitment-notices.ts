...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    return res.status(200).json({ success: true, data: await listRecruitmentNotices() });
  } catch {
    return res.status(500).json({ success: false, data: null, message: 'Không thể tải thông báo tuyển dụng' });
  }
}
