...
const RecruitmentPage: NextPage = () => {
  ...
  return <Layout title="Thông báo tuyển dụng - Viên Chức 247"><section className="recruitment-page"><div className="recruitment-heading"><span className="home-section-kicker">Cập nhật cơ hội mới</span><h1>Thông báo tuyển dụng viên chức giáo dục</h1><p>Các thông báo được tổng hợp từ nguồn tuyển dụng và liên kết tới trang gốc.</p></div><div className="recruitment-filters"><input className="form-input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm thông báo..." /><select className="form-select" value={province} onChange={(event) => setProvince(event.target.value)}><option value="">Tất cả tỉnh/thành</option>{provinces.map((item) => <option value={item} key={item}>{item}</option>)}</select></div>{loading ? <div className="loading-state"><div className="spinner" /><p>Đang tải thông báo...</p></div> : filtered.length === 0 ? <div className="empty-state"><p>Chưa có thông báo phù hợp.</p></div> : <div className="recruitment-list">{filtered.map((notice) => <article className="recruitment-card" key={notice.id}><div className="recruitment-card-meta"><span>{notice.source_name || 'Nguồn tổng hợp'}</span>{notice.province && <span>{notice.province}</span>}<time>{notice.published_at ? new Date(notice.published_at).toLocaleDateString('vi-VN') : ''}</time></div><h2>{notice.title}</h2>{notice.excerpt && <p>{notice.excerpt}</p>}<a href={notice.official_url || notice.aggregator_url} target="_blank" rel="noreferrer" className="article-read-link">Xem thông báo gốc <span aria-hidden="true">→</span></a></article>)}</div>}</section></Layout>;
};
